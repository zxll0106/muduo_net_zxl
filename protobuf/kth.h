...
getKth(std::function<void(int64_t, int64_t *, int64_t *)> search,
       const int64_t k, const int64_t count, int64_t min, int64_t max) {
    int steps = 0;
    int64_t guess = max;
    bool succeed = false;
    while (min <= max) {
        ...
        LOG_INFO << "guess = " << guess << ", smaller = " << smaller
                 << ", same = " << same << ", min = " << min
                 << ", max = " << max;
