...
void test_down_pointer_cast() {
    ...
    ::std::shared_ptr<muduo::Query> query(
        muduo::down_pointer_cast<muduo::Query>(msg));
    assert(msg && query);
    if (!query) {
        abort();
    }
...
void onQuery(const muduo::TcpConnectionPtr &, const QueryPtr &message,
             muduo::Timestamp) {
...
void onUnknownMessageType(const muduo::TcpConnectionPtr &,
