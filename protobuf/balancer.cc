...
class BackendSession : noncopyable {
  public:
    BackendSession(EventLoop *loop, const InetAddress &backendAddr,
                   const string &name)
        : loop_(loop), client_(loop, backendAddr, name),
  ...
  private:
    ...
    void onRpcMessage(const TcpConnectionPtr &, const RpcMessagePtr &msg,
                      Timestamp) {
        loop_->assertInLoopThread();
        std::map<uint64_t, Request>::iterator it =
            outstandings_.find(msg->id());
...
class Balancer : noncopyable {
  public:
    Balancer(EventLoop *loop, const InetAddress &listenAddr, const string &name,
    ...
    void initPerThread(EventLoop *ioLoop) {
        ...
        for (size_t i = 0; i < backends_.size(); ++i) {
            ...
            snprintf(buf, sizeof buf, "%s#%d", backends_[i].toIpPort().c_str(),
                     count);
            t.backends.emplace_back(
                new BackendSession(ioLoop, backends_[i], buf));
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 3) {
        fprintf(stderr,
                "Usage: %s listen_port backend_ip:port [backend_ip:port]\n",
                argv[0]);
    } else {
