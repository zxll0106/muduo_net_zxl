...
void testQuery() {
    ...
    std::shared_ptr<muduo::Query> newQuery =
        down_pointer_cast<muduo::Query>(message);
...
void testAnswer() {
    ...
    std::shared_ptr<muduo::Answer> newAnswer =
        down_pointer_cast<muduo::Answer>(message);
...
void testBadBuffer() {
...

    {
        ...
        ProtobufCodec::ErrorCode errorCode = ProtobufCodec::kNoError;
        MessagePtr message =
            ProtobufCodec::parse(data.c_str(), len - 1, &errorCode);
        assert(message == NULL);
...

    {
        ...
        data[len - 1]++;
        MessagePtr message =
            ProtobufCodec::parse(data.c_str(), len, &errorCode);
        assert(message == NULL);
        assert(errorCode == ProtobufCodec::kCheckSumError);
    }
...
void onMessage(const muduo::TcpConnectionPtr &conn, const MessagePtr &message,
               muduo::Timestamp receiveTime) {
    g_count++;
}

void testOnMessage() {
    printf("testOnMessage\n");
    muduo::Query query;
    query.set_id(1);
    query.set_questioner("Chen Shuo");
    query.add_question("Running?");
